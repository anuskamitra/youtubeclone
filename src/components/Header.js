...
import ytmobile from "../images/yt-logo-mobile.png"
...
export default function Header() {
  ...
  const searchQueryHandler=(event)=>{
    if((event?.key==="Enter"||event==="searchButton")&& searchQuery?.length>0){
       //?. take optional chaining bole mane let event e kichu nei null ache tahole null.key error debe tai optional chaining korano hoe tate event jodi thake tobei tar key er properties dekhbo
       console.log(event);
  ...
  return (
    <div className="sticky top-0 z-10 flex flex-row items-center justify-between h-14 py-4 px-4 md:px-5 bg-black">
      {loading && <Loader/>}
